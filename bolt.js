(function() {
  ...
  const handleBoltCmd = async function(event) {
    const targetElement = event.target;
    const boltCmd = targetElement.getAttribute("bolt-cmd");
    if (boltCmd) {
      const boltTrigger = targetElement.getAttribute("bolt-trigger");
      if (boltTrigger === "load" || !boltTrigger) {
        try {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    document.body.addEventListener("click", handleBoltCmd);
  });
